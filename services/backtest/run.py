...
)  # Changed default host to localhost for dev
def dev(port, reload, host):
    """Run the backtesting service in development mode."""
    reload_flag = "--reload" if reload else ""
    os.system(f"uvicorn src.backtest_API:app --host {host} {reload_flag} --port {port}")
...
def prod(port, host):
    ...
    os.system(f"uvicorn src.backtest_API:app --host {host} --port {port}")
