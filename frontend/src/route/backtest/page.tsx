...
import { format, toDate, addDays, addHours, addMinutes } from "date-fns";
...
const BacktestPage: React.FC = () => {
	...
	const renderChart = () => {
		...
		if (backtestType === "vectorized") {
			const startDate = form.watch("fetchInput.startDate");
			const interval = form.watch("fetchInput.interval");

			// Map result.data to use actual dates
			const chartData = result.data.map((item, index) => {
				let date = new Date(startDate);
				if (interval === "1d") {
					date = addDays(date, index);
				} else if (interval === "1h") {
					date = addHours(date, index);
				} else if (interval === "5m") {
					date = addMinutes(date, index * 5);
				} else if (interval === "1m") {
					date = addMinutes(date, index);
				}
				return {
