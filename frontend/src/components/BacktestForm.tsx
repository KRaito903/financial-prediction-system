...
interface BacktestFormProps {
	...
	setBacktestType: (
		type: "vectorized" | "event-driven" | "custom-model"
	) => void;
	coinList: string[];
...
const BacktestForm: React.FC<BacktestFormProps> = ({
	...
	const handleSubmit = async (data: BacktestFormData) => {
		...
		try {
			...
			toast.error(`Backtest failed: ${errorMessage}`, {
				duration: 5000,
					...
					<div className="space-y-4">
						<div className="space-y-2">
							...
							<p className="text-sm text-muted-foreground">
								Select a model and scaler from your uploaded
								files below. Click on the files to select them.
							</p>
